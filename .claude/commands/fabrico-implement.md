...
4. **Confirm dev server URL** — If your UI verification inventory from step 3 contains ANY tasks, use the **AskUserQuestion** tool **now** to ask the user for the dev server URL (e.g., "What URL is the frontend app running at?"). Do not defer this — you need the confirmed URL before any UI verification can start. Do not guess from running processes or port scans. Store the confirmed URL for all subsequent verifications.
...
6. **Confirm with user before implementation** — Confirm with the user before proceeding to the implementation phase after research and planning phases using the **AskUserQuestion** tool.
...
7. **Process each task in plan order.** For each task, based on its type:
   ...
   - **`[REUSE]` — UI verification tasks** → These tasks MUST be processed — do NOT skip them. Delegate each one to the `fabrico-ui-reviewer` subagent via the **Task** tool (subagent_type: `fabrico-ui-reviewer`) with `/fabrico-review-ui` (`.claude/commands/fabrico-review-ui.md`), passing: the Figma URL (for the **figma** MCP server (tools `mcp__figma__*`) — to extract design specs), the confirmed dev server URL from step 4 (for the **playwright** MCP server (tools `mcp__playwright__*`) — to capture actual implementation), and the component/section name. For the complete verification workflow (fix→verify iteration loop, confidence handling, escalation rules, verification gate), follow `/fabrico-implement-ui` (`.claude/commands/fabrico-implement-ui.md`).
...
<!-- FABRICO_COLLECTIONS:command:fabrico-implement:v1 -->
