---
description: "Turn a one-line product idea into a complete, build-ready SPEC.md — the format /fabrico-autopilot builds from. Asks a few high-leverage clarifying questions, then fills the full spec template with concrete user stories and acceptance criteria."
argument-hint: "[one-line product idea, e.g. \"a management app for an amateur football club\"]"
---

The product idea: $ARGUMENTS

## Goal

Produce a complete `SPEC.md` from a short idea, so the user can hand it straight to
[`/fabrico-autopilot`](fabrico-autopilot.md) without writing the spec by hand. The spec must follow the structure of
`SPEC.template.md` and be concrete enough that an autonomous build rarely has to stop and ask.
...
1. **Get the idea.** Read it from `$ARGUMENTS`. If empty, ask the user for a one-line product idea (AskUserQuestion).

2. **Ask only the high-leverage questions.** Use the **AskUserQuestion** tool to batch 2–4 questions that
...
3. **Draft the full spec.** Fill **every** section of `SPEC.template.md`:
...
<!-- FABRICO_COLLECTIONS:command:fabrico-create-spec:v1 -->
