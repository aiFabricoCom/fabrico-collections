...
Use the **context7** MCP server (tools `mcp__context7__*`) to look up platform-specific syntax for the detected platform.
...
**Rule:** Prefer OIDC federation over long-lived access keys. Use the `fabrico-managing-secrets` skill (`.claude/skills/fabrico-managing-secrets/SKILL.md`) for implementation details.
...
1. **Discover context** → Use the `fabrico-technical-context-discovering` skill (`.claude/skills/fabrico-technical-context-discovering/SKILL.md`) to find existing CI patterns
2. **Detect platform** → Check for CI config files listed above
3. **Look up syntax** → Use the **context7** MCP server (tools `mcp__context7__*`) for platform-specific YAML syntax
4. **Choose deployment strategy** → Use decision table above
5. **Configure credentials** → Use the `fabrico-managing-secrets` skill (`.claude/skills/fabrico-managing-secrets/SKILL.md`)
...
- `fabrico-managing-secrets` - For credential configuration
- `fabrico-technical-context-discovering` - For finding existing patterns
