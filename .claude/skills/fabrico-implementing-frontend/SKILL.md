...
- Extract specs from Figma (via the **figma** MCP server, tools `mcp__figma__*`, if available). Identify components, spacing, typography, colors, and interaction states.
...
- If a calling workflow provides a verification loop (e.g., the Engineering Manager runs the `fabrico-ui-reviewer` subagent — via the Task tool, subagent_type: `fabrico-ui-reviewer` — automatically during `/fabrico-implement`), defer to that workflow — do not duplicate verification here.
- If no verification workflow is active, use the `fabrico-ui-verifying` skill directly to compare the implementation against the Figma design.
...
- `fabrico-ui-verifying` - for verifying implementation against Figma designs
- `fabrico-technical-context-discovering` - for understanding project conventions before implementing
- `fabrico-ensuring-accessibility` — to ensure components meet WCAG 2.1 AA standards
- `fabrico-optimizing-frontend` — for performance considerations during component implementation
- `fabrico-implementing-forms` — for form-specific component patterns and validation
- `fabrico-writing-hooks` — for custom hook patterns used within components
- `fabrico-reviewing-frontend` — for frontend-specific code review of implemented components
</content>
