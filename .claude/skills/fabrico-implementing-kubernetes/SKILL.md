...
Use the **context7** MCP server (tools `mcp__context7__*`) to look up Kubernetes API versions and syntax.
...
- `fabrico-implementing-observability` - For K8s monitoring and logging setup
- `fabrico-implementing-ci-cd` - For K8s deployment pipelines
- `fabrico-managing-secrets` - For K8s secret management patterns
- `fabrico-implementing-terraform-modules` - For provisioning K8s clusters
