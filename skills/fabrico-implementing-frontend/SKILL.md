...
- Extract specs from Figma via the **figma** MCP server, if available. Identify components, spacing, typography, colors, and interaction states.
...
- If a calling workflow provides a verification loop (e.g., the Engineering Manager runs the `fabrico-ui-reviewer` subagent — through Codex subagent delegation, custom agent `fabrico-ui-reviewer` — automatically during `$fabrico-implement`), defer to that workflow — do not duplicate verification here.
