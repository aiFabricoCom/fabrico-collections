...
2. **Analyze additional materials and baseline**: Review Figma designs using the **figma** MCP server, read PDF documents using the **pdf-reader** MCP server, analyze the existing codebase using the `fabrico-codebase-analysing` skill, review any other reference documents provided, and load the project baseline if `specifications/projects/<project-name>/task-baseline.md` already exists.
