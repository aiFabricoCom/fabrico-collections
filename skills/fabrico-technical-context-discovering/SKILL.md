...
- Search for the root `AGENTS.md` at the repository root.
- Search for nested `AGENTS.md` files in relevant directories (e.g., `src/`, `tests/`, `e2e/`, `backend/`, feature-specific folders) — a `AGENTS.md` in a subdirectory applies to that subtree.
- Check for `AGENTS.override.md` files that replace guidance at a more specific scope.

If guidance files exist, they are the **primary source of truth** for:
...
- **Use the context7 MCP server** to search for official documentation of the framework/library being used (check project config for exact versions first).
