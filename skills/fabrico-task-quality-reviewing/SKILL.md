...
description: "Find quality gaps and missing edge cases in extracted stories."
---
...
Use the checklist below and track your progress with a visible task plan:
...
Use available research tools (the **context7** MCP server, official documentation, and web search) to find relevant industry patterns. Flag findings with **Medium** or **Low** confidence since they are based on external research rather than explicit workshop discussion.
...
If domain-specific research tools are available (the **context7** MCP server, web search):
...
2. **One suggestion per popup**: Present exactly **one suggestion per focused question**. Each popup must be self-contained — the user should be able to make a decision without needing context from a previous popup. Use a direct user question. For each suggestion, show:
...
For each accepted suggestion, apply the proposed change to `extracted-tasks.md` using file-editing tools:
